...
#ifdef _MSC_VER
    return sz > 0 ? recv(s, (char *)data, sz, 0) : 0;
#else
...
socket_addr(const char * ip, uint16_t port, sockaddr_t * addr) {
    if (!ip || !*ip || !addr)
        RETURN(ERR_PARAM, "check empty ip = %s, port = %hu, addr = %p.", ip, port, addr);
    ...
    if (addr->sin_addr.s_addr == INADDR_NONE) {
        ...
        if (!host || !host->h_addr)
            RETURN(ERR_PARAM, "check ip is error = %s.", ip);
        // 尝试一种, 默认ipv4
...
static inline int _socket_set_enable(socket_t s, int optname) {
    int ov = 1;
    return setsockopt(s, SOL_SOCKET, optname, (char *)&ov, sizeof ov);
}
...
socket_get_error(socket_t s) {
    ...
    int code = getsockopt(s, SOL_SOCKET, SO_ERROR, (char *)&error, &len);
...
socket_recvfrom(socket_t s, void * buf, int len, int flags, sockaddr_t * in, socklen_t * inlen) {
    return recvfrom(s, (char *)buf, len, flags, (struct sockaddr *)in, inlen);
}

inline int
socket_sendto(socket_t s, const void * buf, int len, int flags, const sockaddr_t * to, socklen_t tolen) {
    return sendto(s, (char *)buf, len, flags, (const struct sockaddr *)to, tolen);
}
